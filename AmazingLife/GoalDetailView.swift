...
struct GoalDetailView: View {
    ...
    @State private var isProjectRelationsExpanded = false
    @State private var isSubTasksExpanded = false
    @State private var isRelatedContactsExpanded = false
    @State private var isBackgroundImageExpanded = false
    ...
    var goalDescriptionView: some View {
        VStack(alignment: .leading, spacing: 8) {
            TextField(
                "goal_description_placeholder".localized,
                text: Binding(
                    get: { goal.goalDescription },
                    set: { newValue in
                        let old = goal.goalDescription
                        goal.goalDescription = newValue
                        goal.modifyTime = Date()
                        // 记录描述变更（只在内容实际变化时）
                        if old != newValue {
                            GoalActivityManager.shared.logDescriptionChange(goal: goal, oldDescription: old, modelContext: modelContext)
                        }
                        try? modelContext.save()
                    }
                ),
    ...
    // 目标描述与标签独立卡片
    private var descriptionTagsCardView: some View {
        VStack(alignment: .leading, spacing: 16) {
            // 描述标题
            HStack {
            ...
            HStack(spacing: 8) {
                ...
                Button(action: {
                    withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
                        isProjectRelationsExpanded.toggle()
                    }
                }) {
                    Image(systemName: isProjectRelationsExpanded ? "chevron.down" : "chevron.right")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundColor(Color(UIColor.secondaryLabel))
                        .padding(6)
                }
    ...
    private var relatedContactsCardView: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .center, spacing: 6) {
                ...
                Button(action: {
                    withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
                        isRelatedContactsExpanded.toggle()
                    }
                }) {
                    Image(systemName: isRelatedContactsExpanded ? "chevron.down" : "chevron.right")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundColor(Color(UIColor.secondaryLabel))
                        .padding(6)
                }
    ...
    private var backgroundImageCardView: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .center, spacing: 6) {
                ...
                Button(action: {
                    withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
                        isBackgroundImageExpanded.toggle()
                    }
                }) {
                    Image(systemName: isBackgroundImageExpanded ? "chevron.down" : "chevron.right")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundColor(Color(UIColor.secondaryLabel))
                        .padding(6)
                }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // 新增：目标描述与标签独立卡片，放置在目标关联卡片上方
                descriptionTagsCardView
